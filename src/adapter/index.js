...
class Resource extends SequelizeResource {
  sortField() {
    ...
    return decorated.options &&
      decorated.options.sort &&
      decorated.options.sort.sortBy
      ? decorated.options.sort.sortBy
      : this.titleField();
  }
  ...
  wrapObjects(sequelizeObjects) {
    return sequelizeObjects.map((sequelizeObject) =>
      this.wrapObject(sequelizeObject)
    );
  }
  ...
  async findRelated(record, association, options = {}) {
    ...
    return instance[association.accessors.get](options);
  }
  ...
  getAssociationsByResourceId(resourceId) {
    return Object.values(this.SequelizeModel.associations).filter(
      (association) => association.target.tableName === resourceId
    );
  }
