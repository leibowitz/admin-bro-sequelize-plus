...
const after = async (response, request, context) => {
  if (request && request.method) {
    ...
    if (context.action.name === 'edit' && request.method === 'get') {
      ...
      await Promise.all(
        inlineProperties.map(async (property) => {
    ...
    if (request.method === 'post' && record.isValid()) {
      ...
      const inlines = await Promise.all(
        inlineProperties.map(async (property) => {
          ...
          const references = toResource.getPropertiesOfType(
            'edit',
            'reference'
          );
          const refProperty = references.find(
            (p) => p.property.reference() === context.resource.id()
          );

          // Only delete linked records when they were removed from an existing record
          if (context.action.name === 'edit') {
            await toResource.deleteMissingRecords(
              refProperty.name(),
              record.id(),
              items.map((i) => i.id)
            );
          ...
          if (context.action.name === 'new') {
            newItems = items.map((item) => ({
              ...item,
              [refProperty.name()]: record.id()
            }));
          } else {
            newItems = items;
          }

          const records = newItems.map((item) => toResource.build(item));
          const results = await Promise.all(
            records.map(async (item) => item.save())
          );
          return {
            property,
            records: results
      ...
      if (
        Object.keys(response?.record?.errors).length !== 0 &&
