    ...
    const relatedResources = await Promise.all(
      properties.map(async (property) => {
        ...
        if (context.action.name === 'new' && savedRecord.id()) {
          const parentField =
            property?.options?.custom?.field || property.name();
...
const after = async (response, request, context) => {
  if (request && request.method) {
    ...
    const { _admin, translateMessage } = context;
    if (request.method === 'post' && context.relatedResources) {
      context.relatedResources.forEach((related) => {
        ...
        if (!related.record.isValid()) {
          ...
          const toResource = _admin.findResource(toResourceId);
          const parentField =
            related?.property?.options?.custom?.field || propName;
          response.record.populated[parentField] = {
            params: { [toResource.primaryKeyField()]: related.record.id() }
          };
        }
      });
