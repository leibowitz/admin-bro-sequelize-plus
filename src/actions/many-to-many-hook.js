...
const getFromField = (property) => {
  return property?.options?.custom?.fromField
}

const getResource = (_admin, resource, property) => {
  const fromField = getFromField(property)
  if (fromField) {
    const refField = resource.properties().find(p => p.type() === 'reference' && p.name() === fromField)
    if (refField) {
...
const setResponseItems = async (context, response, property) => {
  ...
  const throughItems = await fromResource.findRelated(fromRecord, association, options)
  ...
  if (items.length !== 0) {
    ...
    response.record.params[propName] = items.map(v => v.params[primaryKeyField || 'id'])
  }
}
...
const after = async (response, request, context) => {
  if (request && request.method) {
    ...
    if (context.action.name == 'edit' && request.method === 'get') {
      ...
      await Promise.all(manyProperties.map(async (property) => {
    ...
    if (request.method === 'post' && record.isValid()) {
      ...
      await Promise.all(manyProperties.map(async (property) => {
        const fromResource = getResource(context._admin, context.resource, property)
        const association = getAssociation(context, fromResource, property)
        const propName = property.name()
        const ids = params[propName] ? params[propName].map(v => parseInt(v)) : []
