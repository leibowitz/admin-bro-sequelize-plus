...
const getResourceId = (property) => {
  return property.custom && property.custom.resourceId
    ? property.custom.resourceId
    : property.name;
};

const getOptionFromRecord = (record, targetField) => {
  return {
    value: targetField ? record.params[targetField] : record.id,
    label: record.title
  };
};
...
const SelectEdit = (props) => {
  ...
  const targetField =
    property.custom && property.custom.targetField
      ? property.custom.targetField
      : null;
